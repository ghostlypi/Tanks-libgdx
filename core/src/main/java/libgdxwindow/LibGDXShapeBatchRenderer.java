...
public class LibGDXShapeBatchRenderer extends BaseShapeBatchRenderer
{
    ...
    public FloatBuffer vertBuffer = BufferUtils.newFloatBuffer(capacity * 3);
    public FloatBuffer colBuffer = BufferUtils.newFloatBuffer(capacity * 4);
    ...
    public LibGDXShapeBatchRenderer(LibGDXWindow window, boolean dynamic)
    {
        super(true);
        this.window = window;
    ...
    public LibGDXShapeBatchRenderer(LibGDXWindow window, ShaderGroup s, boolean dynamic)
    {
        this(window, dynamic);
        this.shader = s;
    ...
    public LibGDXShapeBatchRenderer(LibGDXWindow window)
    {
        this(window, true);
    }

    public LibGDXShapeBatchRenderer(LibGDXWindow window, ShaderGroup s)
    {
        this(window, s, true);
    }
    ...
    public void draw()
    {
        window.setDrawMode(-1, this.depth, this.depthMask, this.glow, 0);
        if (window.drawingShadow && (!depthMask || !depth || glow))
            return;
...
//            window.setTransparentBlendFunc();
    ...
    public void expand()
    {
        ...
        FloatBuffer newVertBuffer = BufferUtils.newFloatBuffer(newCapacity * 3);
        FloatBuffer newColBuffer = BufferUtils.newFloatBuffer(newCapacity * 4);
        HashMap<ShaderGroup.Attribute, FloatBuffer> newAttributeBuffers = new HashMap<>();
        for (ShaderGroup.Attribute a: this.attributeBuffers.keySet())
        {
            newAttributeBuffers.put(a, BufferUtils.newFloatBuffer(newCapacity * a.count));
        }
    ...
    public void endModification()
    {
        ...
        Gdx.gl.glBindBuffer(GL_ARRAY_BUFFER, vertVBO);
        Gdx.gl.glBufferSubData(GL_ARRAY_BUFFER, Float.BYTES * start * 3, Float.BYTES * modifyingSize * 3, this.vertBuffer);
        Gdx.gl.glBindBuffer(GL_ARRAY_BUFFER, colVBO);
        Gdx.gl.glBufferSubData(GL_ARRAY_BUFFER, Float.BYTES * start * 4, Float.BYTES * modifyingSize * 4, this.colBuffer);
        for (ShaderGroup.Attribute a: attributeBuffers.keySet())
        {
            Gdx.gl.glBindBuffer(GL_ARRAY_BUFFER, this.attributeVBOs.get(a));
            Gdx.gl.glBufferSubData(GL_ARRAY_BUFFER, Float.BYTES * start * a.count, Float.BYTES * modifyingSize * a.count, this.attributeBuffers.get(a));
        }
    ...
    public void endAdd()
    {
        ...
        else if (this.initSize < this.size && this.initialized)
        {
            ...
            this.window.vertexBufferSubData(vertVBO, Float.BYTES * this.initSize * 3, this.vertBuffer);
            this.window.vertexBufferSubData(colVBO, Float.BYTES * this.initSize * 4, this.colBuffer);

            for (ShaderGroup.Attribute a : attributeBuffers.keySet())
            {
                this.window.vertexBufferSubData(attributeVBOs.get(a), Float.BYTES * this.initSize * a.count, this.attributeBuffers.get(a));
            }
    ...
    public void delete(IBatchRenderableObject o)
    {
        ...
        this.window.vertexBufferSubData(vertVBO, Float.BYTES * pos * 3, 3 * size);
        this.window.vertexBufferSubData(colVBO, Float.BYTES * pos * 4, 4 * size);

        for (ShaderGroup.Attribute a: attributeBuffers.keySet())
        {
            this.window.vertexBufferSubData(this.attributeVBOs.get(a), Float.BYTES * pos * a.count, a.count * size);
        }
    }
    ...
    public void migrate(IBatchRenderableObject o)
    {
        ...
        for (ShaderGroup.Attribute a: attributeBuffers.keySet())
        {
            this.window.vertexBufferSubData(colVBO, Float.BYTES * pos * a.count, a.count * size);
        }
    }

    public void addAttribute(ShaderGroup.Attribute attribute)
    {
        this.attributeBuffers.put(attribute, BufferUtils.newFloatBuffer(capacity * attribute.count));
